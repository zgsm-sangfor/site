...
const isEnterprisePage = computed(() => router.currentRoute.value.name === 'enterprise')
...
const isProductNavigationActive = computed(() => {
  ...
  return ['cloud', 'CliIndex', 'IdeIndex'].includes(route.name as string)
})
...
const toEnterprise = () => router.push({ name: 'enterprise' })
